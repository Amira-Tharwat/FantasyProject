...
void ReadFromFiles() {
	...
	while (getline(squadfile,line)) {
		...
		for (int i = 2; i <= 16; i++) {
			...
			isss2 >> PlayerId;
			Person::users[userid-1].squad[leagueid].squads[Leagues::leagues[leagueid].Players[PlayerId].PlayerPosition].push_back(Leagues::leagues[leagueid].Players[PlayerId]);
		}
		...
		for (int i = 17; i <= 20; i++) {
			...
			isss3 >>playerid;
			Person::users[userid-1].squad[leagueid].deka.push(Leagues::leagues[leagueid].Players[playerid]);
		}
		istringstream isss4(tokens[21]);
		isss4 >> Person::users[userid-1].squad[leagueid].TotalPoints;
		int playerid;
		istringstream isss5(tokens[22]);
		isss5 >> playerid;
		Person::users[userid-1].squad[leagueid].captain = Leagues::leagues[leagueid].Players[playerid];
		istringstream isss6(tokens[23]);
		isss6 >> playerid;
		Person::users[userid-1].squad[leagueid].ViceCaptain = Leagues::leagues[leagueid].Players[playerid];
		istringstream isss7(tokens[24]);
		isss7 >> Person::users[userid-1].squad[leagueid].tribleCaptain ;
		istringstream isss8(tokens[25]);
		isss8 >> Person::users[userid-1].squad[leagueid].wildCard;
		istringstream isss9(tokens[26]);
		isss9 >> Person::users[userid-1].squad[leagueid].numOfSubestitution;
	}
	while (getline(userroundpointsfile, line)) {
		...
		istringstream isss3(tokens[3]);
		isss3 >> points;
		Person::users[userid].squad[leagueid].RoundPoints[roundid] = points;
	}
...
void WriteInFiles() {
	...
	for (auto i = Person::admins.begin(); i != Person::admins.end();i++) {
	...
	for (auto i = Person::users.begin(); i != Person::users.end(); i++) {
		...
		for (auto j : i->squad) {
			...
			for (auto r : Person::users[i->Id].squad[j.first].RoundPoints) {
